	...
	If(!Empty(oJson["dataIni"]))
		...
		If(!Empty(oJson["codCusto"]) .Or. !Empty(oJson["codDir"]) .Or.  !Empty(oJson["codArea"]) .Or.  !Empty(oJson["codDep"]))
			...
			While !(cAliasTop)->(EoF())
				...
				oRegistro['orc'] := cvaltochar(countOrcado((cAliasTop)->RA_CC, (cAliasTop)->QB_XDEPTO, (cAliasTop)->SRA.RA_CODFUNC, oJson["dataIni"]))

				oRegistro['real'] := cvaltochar(countAtivos((cAliasTop)->RA_CC, (cAliasTop)->QB_XDEPTO, (cAliasTop)->SRA.RA_CODFUNC))
			...
			// 	cQuery := " SELECT SRA.RA_CC CODIGO , CTT.CTT_DESC01 DECRICAO, COUNT(*) AS TT " + CRLF

			// 	If(Empty(oJson["codDep"]))
			// 		cQuery += " ,(SELECT "+cMes+" FROM ZA7020 ZA7 WHERE  D_E_L_E_T_ = '' AND ZA7_CCUST = SRA.RA_CC AND ZA7_CDEPTO = ZA8.ZA8_CODIGO ) AS ORCADO_CC"
			// 	Else
			// 		cQuery += " ,(SELECT "+cMes+" FROM ZA7020 ZA7 WHERE  D_E_L_E_T_ = '' AND ZA7_CCUST = SRA.RA_CC AND ZA7_CDEPTO = '"+oJson["codDep"][i]+"' ) AS ORCADO_CC"
			// 	EndIf
			...
			// 	If(!Empty(oJson["codDep"]))
			// 		cQuery += " AND SQB.QB_XDEPTO = '"+oJson["codDep"][i]+"' " + CRLF
			// 	EndIf
			...
			// 	cQuery += " GROUP BY SRA.RA_CC  , CTT.CTT_DESC01, ZA8.ZA8_CODIGO " + CRLF
			...
			// 		oRegistro['custo'] := EncodeUTF8(AllTrim((cAliasTop)->DECRICAO))

			// 		jFuncionarios := infoFunc( (cAliasTop)->CODIGO, "", "", "", "", oJson["dataIni"])

			// 		nOrcado += (cAliasTop)->ORCADO_CC
			// 		nReal += (cAliasTop)->TT - (jFuncionarios['Ferias'][1] + jFuncionarios['Afastamento'][1] + jFuncionarios['Atestado'][1])
			...
			// oRegistro['orc'] := cvaltochar(nOrcado)
			// oRegistro['real'] := cvaltochar(nReal - nOutros)
			// oRegistro['delta'] := cvaltochar(nOrcado - nReal)
